...
Currently when running npm update, we run into several deprecated package warnings, and a error with node-gyp. if we want the resulting project to be maintainable, we need to adhere to modern standards and use modern dependencies
...
1. Run npm update to see the list of outdated dependencies.
...
Produce a commit that updates the dependencies and resolves any issues caused by the update.
